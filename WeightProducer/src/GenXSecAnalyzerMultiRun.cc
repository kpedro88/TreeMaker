#include "GenXSecAnalyzerMultiRun.h"
#include "FWCore/MessageLogger/interface/MessageLogger.h"
#include "TMath.h"
#include <iostream>
#include <iomanip>

GenXSecAnalyzerMultiRun::GenXSecAnalyzerMultiRun(const edm::ParameterSet& iConfig):
  nMCs_(0),
  hepidwtup_(-9999),
  totalWeightPre_(0),
  thisRunWeightPre_(0),
...
GenXSecAnalyzerMultiRun::~GenXSecAnalyzerMultiRun()
{
}

void
GenXSecAnalyzerMultiRun::beginJob() {
...
void 
GenXSecAnalyzerMultiRun::beginRun(edm::Run const& iRun, edm::EventSetup const&)
{
...
void
GenXSecAnalyzerMultiRun::beginLuminosityBlock(edm::LuminosityBlock const& iLumi, edm::EventSetup const&) {
...
void
GenXSecAnalyzerMultiRun::analyze(const edm::Event&, const edm::EventSetup&)
{
}
...
void
GenXSecAnalyzerMultiRun::endLuminosityBlock(edm::LuminosityBlock const& iLumi, edm::EventSetup const&) {
  ...
  if(hepidwtup_== -1)
    {
      if(theProcesses_size!=1){
        edm::LogError("GenXSecAnalyzerMultiRun::endLuminosityBlock") << "Pure parton shower has thisProcessInfos size!=1";
        return;
      }
    }
  ...
  for(unsigned int ip=0; ip < theProcesses_size; ip++)
    {
      ...
      if(theProcesses[ip].lheXSec().value()<0){
        edm::LogError("GenXSecAnalyzerMultiRun::endLuminosityBlock") << "cross section of process " << ip << " value = "  << theProcesses[ip].lheXSec().value();
        return;
      }
    }
...
void 
GenXSecAnalyzerMultiRun::endRun(edm::Run const& iRun, edm::EventSetup const&)
{
...
void 
GenXSecAnalyzerMultiRun::combine(double& finalValue, double& finalError, double& finalWeight, const double& currentValue, const double& currentError, const double & currentWeight)
{
...
void 
GenXSecAnalyzerMultiRun::combine(GenLumiInfoProduct::XSec& finalXSec, double &totalw, const GenLumiInfoProduct::XSec& thisRunXSec, const double& thisw)
{
...
GenXSecAnalyzerMultiRun::compute(const GenLumiInfoProduct& iLumiInfo)
{
...
void
GenXSecAnalyzerMultiRun::endJob() {

  edm::LogPrint("GenXSecAnalyzerMultiRun") << "\n"
                                   << "------------------------------------" << "\n"
                                   << "GenXSecAnalyzerMultiRun:" << "\n"
                                   << "------------------------------------";
...
DEFINE_FWK_MODULE(GenXSecAnalyzerMultiRun);
